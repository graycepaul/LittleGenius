...
const MATH_MODES = [
  ...
  },
  {
    ...
    label: "Number Bonds",
    emoji: "🔗",
    desc: "What adds up to the total?",
...
function CountingGame({ onBack }) {
  ...
  return (
    <div className="max-w-md mx-auto text-center">
      <button
        onClick={onBack}
        className="text-gray-500 font-semibold mb-6 block hover:text-gray-700"
      >
        ← Back
      </button>
      <div className="bg-white rounded-3xl shadow-xl p-8">
        <h2 className="font-fun text-2xl text-gray-700 mb-6">
          How many {set.name}?
        </h2>
...
function ArithmeticGame({ mode, onBack }) {
  ...
  const [problem, setProblem] = useState(() =>
    mode === "addition" ? generateAddition(10) : generateSubtraction(10),
  );
  const [answer, setAnswer] = useState("");
  const [result, setResult] = useState(null);
  ...
  return (
    <div className="max-w-md mx-auto text-center">
      ...
      {streak > 0 && (
        <p className="text-orange-500 font-fun text-lg mb-2">
          🔥 {streak} in a row!
        </p>
      )}
      <div className="bg-white rounded-3xl shadow-xl p-8">
        ...
        <div className="flex items-center justify-center gap-4 mb-6 flex-wrap">
          ...
          <div className="flex flex-wrap justify-center gap-1 max-w-32">
            {Array(problem.b)
              ...
              .map((e, i) => (
                <span
                  key={i}
                  className={`text-2xl ${mode === "subtraction" ? "opacity-30 line-through" : ""}`}
                >
                  {e}
                </span>
              ))}
          ...
          <AnimatePresence mode="wait">
            <motion.span
              ...
              className={`font-fun text-5xl ${answer ? "text-indigo-500" : "text-yellow-400"}`}
            >
              {answer || "?"}
            </motion.span>
          </AnimatePresence>
        ...
        <p className="font-fun text-4xl text-gray-800 mb-6">
          ...
          <span style={{ color: answer ? "#4F46E5" : "#F59E0B" }}>
            {answer || "?"}
          </span>
        </p>
        ...
        <AnimatePresence>
          {result !== null && (
            ...
            >
              <p
                className={`font-fun text-2xl ${result ? "text-green-500" : "text-red-500"}`}
              >
                {result
                  ? "🎉 Yes! +1 ⭐"
        ...
        <div className="flex gap-3 justify-center">
          ...
          <button
            onClick={next}
            className="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold px-5 py-3 rounded-2xl"
          >
            Skip →
          </button>
        </div>
...
function FillSequence({ onBack }) {
  ...
  useEffect(() => {
    inputRefs.current[seq.missing[0]]?.focus();
  }, [seqIdx]);
  ...
  function check() {
    let correct = 0;
    seq.missing.forEach((n) => {
      if (parseInt(inputs[n]) === n) correct++;
    });
    setScore(correct);
    setChecked(true);
    addStars(correct);
    if (correct === seq.missing.length) {
      triggerStarBurst();
      playCheer();
    }
  }
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="bg-white rounded-3xl shadow-xl p-6">
        <h2 className="font-fun text-2xl text-gray-700 mb-2 text-center">
          Fill the Missing Numbers!
        </h2>
        <p className="text-gray-400 font-semibold text-center mb-6">
          Type the numbers that are missing 👇
        </p>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-6">
          {numbers.map((n) => {
            ...
            return (
              <div
                key={n}
                className={`w-12 h-12 rounded-2xl flex items-center justify-center font-fun text-xl border-2 transition-all
        ...
        <AnimatePresence>
          {checked && (
            ...
            >
              {score === seq.missing.length
                ? `🎉 Perfect! +${score} ⭐`
                : `${score}/${seq.missing.length} correct! Keep going!`}
            </motion.p>
          )}
        </AnimatePresence>
        ...
        <div className="flex gap-3 justify-center">
          {!checked ? (
            <button
              onClick={check}
              className="bg-gradient-to-r from-orange-400 to-yellow-400 text-white font-fun text-xl px-8 py-3 rounded-2xl shadow-lg"
            >
              Check! ✅
            </button>
          ) : (
            <button
              onClick={next}
              className="bg-gradient-to-r from-green-400 to-teal-400 text-white font-fun text-xl px-8 py-3 rounded-2xl shadow-lg"
            >
              Next →
            </button>
          )}
  ...
  return (
    <div className="max-w-md mx-auto text-center">
      ...
      <div className="bg-white rounded-3xl shadow-xl p-8">
        ...
        <div className="flex items-center justify-center gap-6 mb-8">
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              className={`w-16 h-16 rounded-full border-4 font-fun text-3xl flex items-center justify-center
                ${answer ? "bg-indigo-400 border-indigo-500 text-white" : "bg-yellow-300 border-yellow-400 text-gray-700"}`}
            >
              {answer || "?"}
            </motion.div>
          </AnimatePresence>
        ...
        <p className="font-fun text-3xl text-gray-800 mb-6">
          {pair[0]} + ? = {bond.total}
        </p>
  ...
  return (
    <div>
      <PageHeader
        emoji="🔢"
        title="Math World"
        subtitle="Count, add and discover numbers!"
        accentColor="var(--c-math)"
      />
      <AnimatePresence mode="wait">
        {!mode && (
          <motion.div
          ...
          >
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {MATH_MODES.map((m, i) => (
                ...
                >
                  <div
                    className={`bg-gradient-to-br ${m.color} h-24 flex items-center justify-center text-5xl`}
                  >
                    {m.emoji}
                  </div>
                  <div className="p-4">
                    <h3 className="font-fun text-lg text-gray-800">
                      {m.label}
                    </h3>
        ...
        {mode === "counting" && (
          <CountingGame key="counting" onBack={() => setMode(null)} />
        )}
        {mode === "addition" && (
          <ArithmeticGame
            key="addition"
        ...
        {mode === "fill-sequence" && (
          <FillSequence key="fill" onBack={() => setMode(null)} />
        )}
        {mode === "number-bonds" && (
          <NumberBonds key="bonds" onBack={() => setMode(null)} />
        )}
      </AnimatePresence>
    </div>
  );
